...
function generateUUID() {
  ...
  var uuid = "Nxxxxxxxx_xxxx_4xxx_yxxx_xxxxxxxxxxxx".replace(
    /[xy]/g,
    function (c) {
...
function printTree() {
  ...
  console.log(
    rootNode.value,
    "Left Node: " + leftNodeVal,
    "Right Node: " + rightNodeVal
  );
...
var presenter = {
  ...
  addToParent: function (node) {
    if (node.parent) {
      ...
      if (node.value >= parentNode.value) {
        ...
        node.locX =
          parentNode.locX + pixelOffset / Math.pow(1.8, node.depth - 1);
      } else {
    ...
    function fixCollisions(nodeToCheck) {
      ...
      $.each(rootNode.allChildren(), function (index, node) {
        if (
          ...
          nodeToCheck.locX +=
            ((Math.sign(nodeToCheck.locX - node.locX) * closeness) / 2) *
            nodeToCheck.size;
          node.locX +=
            ((Math.sign(node.locX - nodeToCheck.locX) * closeness) / 2) *
            nodeToCheck.size;
  ...
  deleteNode: function (valueOrNode) {
    ...
    model.nodesToAnimate.push([
      { node: deleteNode, animationType: "delete", hide: hide },
    ]);
    var childNodesToAnimate = [],
      replaceNode = null;
  ...
  changeAnimationSpeed: function (value, noAnimation) {
    if (value > 2) {
      model.animationSpeed.delay = 0;
    } else {
      model.animationSpeed.delay = Math.min(
        0,
        -100 * Math.pow(value, 2) + 50 * value + 300
      );
    }
    ...
    var animate = {
      ...
      delete: function (animation) {
        d3.select("#" + animation.node.id + " > .nodeWrap")
          ...
          .each("end", function () {
            ...
            $.each(
              presenter.returnChildren(animation.node),
              function (index, node) {
      ...
      move: function (animation) {
        if (animation.node.parent) {
          $("#" + animation.node.id).insertBefore(
            $("#" + animation.node.parent.id)
          );
        } else {
        ...
        if (animation.lineAnimation) {
          d3.select("#" + animation.node.id + " > .nodeLine")
            .transition()
            .attr({
      ...
      unhide: function (animation) {
        d3.select("#" + animation.node.id + " > .nodeWrap").attr(
          "transform",
          "scale(1)"
        );
        if (animation.drawConnection) {
        ...
        if (animation.newValue) {
          d3.select("#" + animation.node.id + " > .nodeWrap > text").text(
...
var zoomHandler = d3.behavior
  .zoom()
  .scaleExtent([0.5, 10])
  .on("zoom", view.draw);
var svg = d3.select("#svg").call(zoomHandler).append("g").append("g");
